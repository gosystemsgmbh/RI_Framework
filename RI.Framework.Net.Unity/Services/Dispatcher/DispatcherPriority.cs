...
namespace RI.Framework.Services.Dispatcher
{
    ...
    /// <remarks>
    ///     <para>
    ///         <see cref="Now" />: The operation is executed immediately when dispatched.
    ///         The operation is executed in the same thread it is dispatched from.
    ///     </para>
    ///     <para>
    ///         <see cref="Frame" />:  The operation is executed during the current frame.
    ///         The operation is executed in the main/foreground thread.
    ///     </para>
    ///     <para>
    ///         <see cref="Later" />: The operation is executed during the frame which had no other dispatcher operations of higher priority (<see cref="Now" /> or <see cref="Frame" />).
    ///         The operation is executed in the main/foreground thread.
    ///     </para>
    ///     <para>
    ///         <see cref="Idle" />: The operation is executed during the frame which had no other dispatcher operations of higher priority (<see cref="Now" />, <see cref="Frame" />, or <see cref="Later" />) and whose previous frame had no dispatcher operations at all.
    ///         The operation is executed in the main/foreground thread.
    ///     </para>
    ///     <para>
    ///         <see cref="Background" />: The operation is executed in a background thread, using <see cref="ThreadPool" />.
