...
namespace RI.Framework.IO.Paths
{
	...
	///         For invalid paths, an error description is provided (using <see cref="Error" />).
	///     </para>
	///     <para>
	                                     ...
	                                     IComparable
	{
		...
		public static PathProperties FromPath (string path, bool allowWildcards, bool allowRelatives, PathType? assumedType)
		{
			...
			if (type != PathType.Invalid)
			{
				List<string> parts = path.Split(PathProperties.UnixDirectorySeparator, PathProperties.WindowsDirectorySeparator, PathProperties.UncDirectorySeparator).ToList();
