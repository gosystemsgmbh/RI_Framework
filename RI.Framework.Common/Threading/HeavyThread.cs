                ...
                try
                {
                    ...
                    using (ManualResetEvent startEvent = new ManualResetEvent(false))
                    {
                        ...
                        lock (this.SyncRoot)
                        {
                            ...
                            if (!started)
                            {
                                throw new TimeoutException(this.GetType().Name + " failed to start (timeout of " + timeout.ToString("D", CultureInfo.InvariantCulture) + " ms while waiting for start event).");
                            }
